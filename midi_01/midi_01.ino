...
void loop() {
  ...
  if (ldrIsActive) {
    ...
    if (abs(ldrValue - lastLdrValue) >= 1) {                                            // limit the sensor output by using a thresshold
      //sendMidi(0xB0, 0, ldrValue);    // this caused non-wanted effect
  ...
  //for ( int i = 0; i < numberOfInputs; i++) {
   for ( int i = 0; i < 1; i++) {
    ...
    if (buttonState06 == LOW) {
      sendMidi(0x90, 80, NOTE_VELOCITY);
    } else {
      sendMidi(0x91, 80, NOTE_VELOCITY);
    }
    
    if (buttonState07 == LOW) {
      sendMidi(0x90, 70, NOTE_VELOCITY);
    }
    else {
      sendMidi(0x91, 70, NOTE_VELOCITY);
    }
